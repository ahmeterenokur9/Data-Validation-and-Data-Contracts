...
-   **Bidirectional Communication & Actuator Control**: Extends beyond data ingestion to send validated commands to actuators (e.g., smart lamps), enabling a full IoT control loop.
...
![Actuator Data Flow](images/actuator_flow.jpeg)
...
3.  The application reads the `config.json` and `actuator_config.json` files to get the initial MQTT broker settings, sensor topic mappings, and actuator configurations.
4.  A new instance of the `MQTTClient` class from `mqtt_manager.py` is created. This client is initialized with both the sensor and actuator mappings.
...
6.  The `MQTTClient` connects to the broker and, upon a successful connection (`on_connect` callback), it subscribes to every `source` topic defined in the topic mappings (e.g., `/sensor1`) as well as all relevant actuator topics (e.g., `/smart_lamp/command` and `/smart_lamp/status`).
7.  At this point, the system is fully operational and actively listening for incoming data and ready to process commands.

### Stage 2: The Journey of a Single Sensor Message
...
### Stage 2B: The Journey of an Actuator Command & Status Update

This pipeline isn't just for listening; it's also for commanding. The system uses a similar validation-first approach to ensure that only well-formed, safe commands are sent to physical devices.

#### Command Flow (e.g., Turning on a Light)

1.  **Command Publication**: An external entity (like a user clicking a button in a separate UI, or an automated script) publishes a command payload to a raw command topic, e.g., `{"command": "blue", "room": "living_room"}` to `/smart_lamp/command`.

2.  **Command Ingestion & Validation**: The `MQTTClient` receives this message. It looks up the configuration for this actuator and finds its associated `command_schema` (e.g., `schemas/actuators/actuator_smart_lamp_command.json`). The command is validated against this schema to ensure all required fields are present and correctly typed.

3.  **Bifurcation (Command)**:
    -   **If Command Succeeds**:
        1.  The valid command is republished to a **validated** topic (e.g., `/smart_lamp/command/validated`). The actual actuator device (e.g., `smart_lamp_simulator2.py`) is subscribed only to this topic, ensuring it never receives malformed commands.
        2.  The `mqtt_actuator_messages_processed_total` Prometheus `Counter` is incremented with labels `type="command"` and `status="validated"`.
    -   **If Command Fails**:
        1.  An error report is generated and published to a **failed** topic (e.g., `/smart_lamp/command/failed`).
        2.  The Prometheus `Counter` is incremented with `status="failed"` and the specific `error_type`.

#### Status Flow (e.g., The Light Reports its State)

1.  **Status Publication**: After executing a command (or periodically), the actuator publishes its current state to its status topic, e.g., `{"actuator_id": "smart_lamp", "room": "living_room", "status": "blue"}` to `/smart_lamp/status`.

2.  **Status Ingestion & Validation**: The `MQTTClient` receives this status update, validates it against the `status_schema`, and republishes it to a validated topic.

3.  **Update Monitoring**: The `actuator_last_known_state` Prometheus `Enum` metric is updated to reflect the new state (`actuator_id="smart_lamp"`, `room="living_room"`, `state="blue"`). This is what powers our Grafana dashboard for actuator states.
...
This is where you define the core routing and validation logic of the pipeline. Each "mapping" is a card that represents a contract for a specific data source. It is primarily used for sensor data routing. Actuator configurations are currently managed via the `actuator_config.json` file.
...
│   ├── actuators/        # Schemas specifically for validating actuator commands and statuses.
│   └── ...
├── sensors/              # Includes various Python scripts that simulate sensor data publication for testing.
├── actuators/            # Scripts that simulate actuators (e.g., smart lamps) and react to commands.
├── static/               # Frontend assets (HTML, CSS, JS) for the Admin Panel and Live Dashboard.
├── config.json           # The heart of the system's configuration: MQTT settings and sensor topic mappings.
├── actuator_config.json  # Configuration file for defining actuators and their associated MQTT topics and schemas.
...
-   **Support for Advanced Pandera Features**: The validation logic is currently built around dynamically loading `DataFrameSchema` definitions. The system could be extended to support other powerful Pandera features, offering alternative, code-centric validation approaches. This could include:
    ...
    -   **Support for Schema-Level Checks**: Enhancing the dynamic JSON-to-schema builder to support schema-level checks. This would allow for defining validation rules that involve relationships between multiple columns (e.g., ensuring `start_time` is always before `end_time`), which is a powerful feature for complex data contracts.
