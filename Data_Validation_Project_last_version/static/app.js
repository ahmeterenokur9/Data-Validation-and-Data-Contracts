...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const createMessageCard = (topic, message) => {
        ...
        if (status === 'failed' && data.errors) {
            ...
            const newTypesFound = updateErrorTypeFilters(errorTypes);
            // If new filters were added, re-apply them to show the new card correctly
            if (newTypesFound) {
                 // The main applyFilters call will handle this later, no need to call here
            }
        }
    ...
    const applyFilters = () => {
        ...
        // We now select all error checkboxes, including dynamically added ones
